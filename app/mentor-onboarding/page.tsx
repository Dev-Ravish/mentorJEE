...
export default function MentorOnboarding() {
    ...
    async function checkIfMentor(email: string) {
        try {
            const res = await fetch(`/api/validateMentor?email=${encodeURIComponent(email)}`)
        ...
        <div className=' h-full py-8'>
            <SignedOut>
                <div className='flex flex-col items-center justify-center h-full py-8'>
                    ...
                    <p className='mt-4'>
                        Don&apos;t have an account? <SignUpButton mode="modal"><Button>Sign Up</Button></SignUpButton>
                    </p>
                </div>
            </SignedOut>
            ...
            <SignedIn>
                {!isValidMentor() ? (
                    ...
                    <div className='flex flex-col items-center justify-center  h-full py-8'>
                        <h1 className='text-xl font-medium text-gray-600'>Checking mentor status...</h1>
                    </div>
                ) : isMentor ? (
                    <div className='flex flex-col items-center justify-center h-full py-8'>
                        <h1 className='text-2xl font-bold mb-4'>You are already registered as a mentor!</h1>
                    </div>
                ) : (
