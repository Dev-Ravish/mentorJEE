...
export default function MentorRegistrationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.email || !formData.bio || !formData.image || !formData.oneLiner || !formData.upiId) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-md space-y-6">
      ...
      <div>
        ...
        <Input name="oneLiner" value={formData.oneLiner} onChange={handleInputChange} required />
      </div>

      <div>
        <Label>Bio</Label>
        <Textarea name="bio" value={formData.bio} onChange={handleInputChange} rows={4} required />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {formData.tags.map(tag => (
            ...
            >
              {tag}
              <button type="button" onClick={() => handleRemoveTag(tag)} className="text-xs ml-1">✕</button>
            </span>
          ))}
        </div>
      </div>

      <Button type="submit" className="bg-[#2F2D9E] hover:bg-[#3f3dc4] text-white">
        Submit
      </Button>
    </form>
  );
